import { User as UserType, Role, BrokerCompany, Permission, Agent, Broker } from '@/types';
...
export class BrokerStore {
  ...
  async create(input: Omit<Broker, 'id' | 'createdAt' | 'updatedAt'>): Promise<Broker> {
    ...
    const broker: Omit<Broker, 'id'> = {
      ...input,
      createdAt: now,
      updatedAt: now
    };

    const savedBroker = await this.saveToDB(broker as Broker);
    if (savedBroker) {
...
export class UserStore {
  ...
  async createUser(user: Partial<UserType>): Promise<UserType | undefined> {
    const { data, error } = await supabase
      .from('User')
      .insert([user])
      .select('*, role:Role(*), broker:Broker(*)')
      .single();
