...
// 默认空密码的 bcrypt hash（10轮加盐）
const DEFAULT_PASSWORD_HASH = '$2b$10$CwTycUXWue0Thq9StjUM0uJ8iF1rQ4u1l6a1Yy0eQ8e6v9QW5lYyK';
...
export default function TeamManagement() {
  ...
  const handleSendInvite = async (agentId: string) => {
    ...
    // 直接用 agent.firstName/agent.lastName
    ...
    try {
      ...
      if (!userId) {
        ...
        if (roleError || !agentRole) {
          toast.error('No Agent role found');
          return;
        }
    ...
    return users.map(user => (
      ...
      >
        <CardHeader className="flex flex-row items-center justify-between py-4">
          <div>
            <CardTitle className="text-lg text-gray-900">{user.firstName || user.lastName ? `${user.firstName ?? ''} ${user.lastName ?? ''}`.trim() : user.email}</CardTitle>
                ...
                {users.map(user => (
                  ...
                  >
                    <CardHeader className="flex flex-row items-center justify-between py-6">
                      <div>
                        <CardTitle className="text-xl text-gray-900">{user.firstName || user.lastName ? `${user.firstName ?? ''} ${user.lastName ?? ''}`.trim() : user.email}</CardTitle>
